...
import { computeResults, createExam, createGradingScale, examCreateSchema, gradingScaleSchema, marksSaveSchema, publishResults, saveMarks, setExamGradingScale } from '@/server/modules/exams/service'
...
export async function createExamAction(_previous: FormState, formData: FormData): Promise<FormState> {
  try {
    ...
    const exam = await createExam(ctx, examCreateSchema.parse({
      name: formData.get('name'), kind: formData.get('kind'), startsOn: formData.get('startsOn') || undefined,
      endsOn: formData.get('endsOn') || undefined,
      classLevelIds: formData.getAll('classLevelIds'), classSubjectIds: formData.getAll('classSubjectIds'),
    }))
    revalidatePath('/exams')
    redirect(`/exams/${exam.id}/marks`)
  } catch (error) {
...
export async function createGradingScaleAction(_previous: FormState, formData: FormData): Promise<FormState> {
  try {
    ...
    const bands = [0, 1, 2, 3, 4].map((index) => ({
      grade: formData.get(`grade-${index}`), minPercent: formData.get(`min-${index}`), maxPercent: formData.get(`max-${index}`),
      points: formData.get(`points-${index}`) || null, remark: formData.get(`remark-${index}`) || null,
      isPass: formData.get(`pass-${index}`) === 'on',
    }))
    await createGradingScale(ctx, gradingScaleSchema.parse({ name: formData.get('name'), isDefault: formData.get('isDefault') === 'on', bands }))
...
export async function computeResultsAction(examId: string): Promise<{ ok: boolean; message: string }> {
  try {
    ...
    return { ok: true, message: `${result.calculated} results computed${result.skipped ? `; ${result.skipped} students still need complete marks.` : '.'}` }
  } catch (error) { return { ok: false, message: error instanceof Error ? error.message : 'Could not compute results' } }
}
...
export async function publishResultsAction(examId: string): Promise<{ ok: boolean; message: string }> {
  ...
  } catch (error) { return { ok: false, message: error instanceof Error ? error.message : 'Could not publish results' } }
}

export async function setExamGradingScaleAction(examId: string, gradingScaleId: string): Promise<{ ok: boolean; message: string }> {
  try {
  ...
  } catch (error) { return { ok: false, message: error instanceof Error ? error.message : 'Could not update the grading scale' } }
}
