...
export default async function AdmitCardsIndexPage({
  ...
  const ctx = await requireContext('exams.admit_cards')
  ...
  return (
    <div>
      <PageHeader
        title="Admit cards"
        description="Choose an examination, then generate and approve admit cards by section."
      />
      ...
      <Card className="overflow-hidden">
        ...
        {open.length === 0 ? (
          <EmptyState
            title={params.q ? 'No exams match that search' : 'No examinations ready for admit cards'}
            description="Create an exam first, then return here to issue admit cards."
            action={
